...
//        case ex: Throwable if ops.size > 10 => fail(s"failure to receive confirmation $i/${ops.size}", ex)
  ...
  test("contains test") {
    ...
    topNode ! Insert(testActor, id = 1, 3)
    topNode ! Contains(testActor, id = 2, 3)
  ...
  test("insert,remove,insert") {
    
    val requester = TestProbe()
    val requesterRef = requester.ref
    val ops = List(
      Insert(requesterRef, 1, 11),
      Insert(requesterRef, 2, 11),
      Remove(requesterRef, 3, 11),
      Insert(requesterRef, 4, 11),
      Contains(requesterRef, 5, 11))
  ...
  test("simple") {
    ...
    val requesterRef = requester.ref
    val ops = List(
      Remove(requesterRef, 0, 19),
      Insert(requesterRef, 1, 58),
      Insert(requesterRef, 2, 10),
      Insert(requesterRef, 3, 61),
      Remove(requesterRef, 4, 51))
 ...
 ignore("behave identically to built-in set, excludes GC") {
    val rnd = new Random()
